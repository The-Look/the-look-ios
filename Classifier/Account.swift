...
class Account: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func getUserDetails() {
        ...
        if currUser[USER_ABOUT_ME] != nil {
            aboutMeTxt.text = currUser[USER_ABOUT_ME] as? String == "" ? "Write something about yourself." : "\(currUser[USER_ABOUT_ME]!)"
        }
        else
        {
            aboutMeTxt.text = "Write something about yourself."
        ...
        if currUser[USER_EMAIL_VERIFIED] != nil {
            self.verifiedLabel.text = currUser[USER_EMAIL_VERIFIED] as? Bool == true ? "Verified Profile: Yes" : "Verified Profile: No"
        } else {
